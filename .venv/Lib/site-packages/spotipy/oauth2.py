...
def _make_authorization_headers(client_id, client_secret):
    # Convert client_id and client_secret to strings if they're tuples
    client_id_str = ''.join(str(item) for item in client_id) if isinstance(client_id, tuple) else str(client_id)
    client_secret_str = ''.join(str(item) for item in client_secret) if isinstance(client_secret, tuple) else str(client_secret)

    auth_header = base64.b64encode(
        f"{client_id_str}:{client_secret_str}".encode("ascii")
    )
